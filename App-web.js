...
import { Home, BarChart3, Timer, BookOpen, Plus } from 'lucide-react-native';
...
function WebApp() {
  ...
  const renderScreen = () => {
    if (modalScreen === 'AddSession') {
      return <AddSessionScreen onClose={() => setModalScreen(null)} />;
    }
    if (modalScreen === 'EditSession') {
      return <EditSessionScreen sessionId={sessionId} onClose={() => setModalScreen(null)} />;
    }

    switch (currentScreen) {
      case 'Home':
        return <DashboardScreen onAddSession={() => setModalScreen('AddSession')} onEditSession={(id) => { setSessionId(id); setModalScreen('EditSession'); }} />;
      case 'Training Log':
        return <LogScreen onAddSession={() => setModalScreen('AddSession')} onEditSession={(id) => { setSessionId(id); setModalScreen('EditSession'); }} />;
      case 'Statistics':
        return <StatsScreen />;
      case 'Round Timer':
        return <TimerScreen />;
